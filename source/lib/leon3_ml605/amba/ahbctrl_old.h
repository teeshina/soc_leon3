...
//      int lsplmst;//      : integer range 0 to nahbmx-1; rbSplitMaster
    ...
    TDFF<uint32> rbSplit[AHB_MASTERS_TOTAL_2n];
    TDFF<uint32> rbSplitMaster;
    TDFF<uint32> rbMstSize; //hsize  : std_logic_vector(2 downto 0);

    
    static const int primst = AHB_MASTER_HIGHEST;// AHB_MASTERS_MAX downto 0 = [16:0]
  ...
  private:
    uint32 tz(uint32 inVect[2*AHB_MASTERS_TOTAL_2n],
              uint32 inVectLength);
                   
    uint32 lz(uint32 vect_in, int vec_length);

    uint32 wbAhbBusRequest[AHB_MASTERS_TOTAL_2n];
    uint32 wbAhbSplit[AHB_MASTERS_TOTAL_2n];
